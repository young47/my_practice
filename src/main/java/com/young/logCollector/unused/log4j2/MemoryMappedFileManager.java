...
public class MemoryMappedFileManager extends OutputStreamManager {
    ...
    protected MemoryMappedFileManager(final RandomAccessFile file, final String fileName, final OutputStream os,
        ...
        this.nextIndex = getLastIndex(fileName);
    }
    ...
    private synchronized void remap() {
        ...
        final int lastPos = mappedBuffer.limit();
        try {
            final long fileLength = mappedBuffer.position();
            unsafeUnmap(mappedBuffer);
            randomAccessFile.setLength(fileLength);
            rollingOver();
            mappedBuffer = mmap(randomAccessFile.getChannel(), getFileName(), this.mappingOffset, regionLength);
            this.byteBuffer = mappedBuffer;
            LOGGER.debug("create new mapped file:{},", getFileName());
        } catch (final Exception ex) {
            logError("Unable to remap", ex);
        }
    }

    private void rollingOver() throws IOException {
        File fileWhichIsFull = new File(getFileName());
        File historyFile = new File(getFileName() + "." + (nextIndex++));
        fileWhichIsFull.renameTo(historyFile);
        LOGGER.debug("{} rename mapped file to {},", getFileName(), historyFile.getName());

        fileWhichIsFull.createNewFile();

        RandomAccessFile oldRaf = randomAccessFile;
        randomAccessFile = new RandomAccessFile(fileWhichIsFull, "rw");
        //关闭旧的file
        oldRaf.close();
    }

    private String nextMappedFile(int nameIndex) throws IOException {
        String fileWhichIsFull = getFileName();
        File parentFile = new File(fileWhichIsFull).getParentFile();
        String newFile = parentFile + File.separator + nameIndex + ".log";
        RandomAccessFile oldRaf = randomAccessFile;
